...
{data %>% group_by(ID,donnee_contexte_commune) %>% summarize(nb_repondants_commune = sum(inclusion_enquete))
}
...
```{r}
distance <- sqrt(sum((center1 - center2)^2 * data[group1, "pop"] * data[group2, "pop"]))
```
