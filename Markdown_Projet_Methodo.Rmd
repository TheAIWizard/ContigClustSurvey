...
Nous induisons une corrélation positive entre la proportion étudiée (donnée de contexte) et le nombre de répondants pour simuler un comportement particulier (pouvant être sociologique) de la population : par exemple le fait qu'il y ait une propension à répondre à l'enquête pour les gens ayant une valeur plus élevée de la donnée de contexte considérée.s
...
Pour effectuer un tirage à deux degrés, nous reprenons exactement le même principe en filtrant sur les lignes correspondantes à celles se trouvant dans les unités primaires sélectionnées. Pour cela, nous créons une indicatrice de sélection de l'individu en tant qu'unité secondaire localisée dans l'unité primaire sélectionnée.

##### Définition des zones d'absence d'enquêté

Cela nous permet de définir un pourcentage de communes sans-répondants. À partir de cette information, nous en déduisons le pourcentage de communes possédant des répondants à l'enquête. L'utilisateur peut modifier cette valeur.
...
Si l'utilisateur souhaite supprimer certaines communes de l'enquête, il est aussi possible de définir la zone manuellement pour être sûr d'avoir un bloc d'absence de répondants.

Le tirage à deux degrés, nous a justement permis d'automatiser en prenant un certain nombre de communes voisines ne se trouvant pas en métropole ou dans d'autres zones souhaitées. L'option manuelle est à 

Si c'est ce que l'on souhaite, l'utilisateur doit créer une variable d'appartenance à une métropole et reprendre le principe de notre ligne de code ci-dessous.
...
# Proportion des communes exempts de répondants à l'enquête
taux_absence_enquete_commune <- 0.5

# Nombre d'unités primaires à tirer déduites du pourcentage de communes sans répondants
n_up_sample <- floor((1-taux_absence_enquete_commune)*n_squares)

# Tirage des unités primaires au hasard
up_sample <- sample(unique(donnees$ID), n_up_sample)

# Indicatrice de sélection des individus associés aux ID de communes sélectionnées dans up_sample
donnees$inclusion_enquete_up <- ifelse(donnees$ID %in% up_sample,1,0)

# Sous-échantillonnage des unités secondaires pour chaque unité primaire sélectionnée
# Effectuer le tirage de Bernoulli selon les données de contexte pour chaque carré
donnees$inclusion_enquete <- ifelse(donnees$inclusion_enquete_up==1,rbinom(nrow(donnees), 1, donnees$donnee_contexte_commune),0)
...
Nous avons une structure des données de contexte qui affecte positivement le nombre de répondants. Observe t-on une tendance positive entre les données de contexte et le nombre de répondants ? 

```{r}
plot(donnees_agg$nb_repondants_commune,donnees_agg$donnee_contexte_commune, main="Scatterplot du lien entre données de contexte et nombre de répondants", sub="Constatation de la corrélation",
  xlab="Nombre de répondants", ylab="Valeurs des données de contexte")
```
La tendance ne semble peut-être pas purement linéaire mais nous constatons une tendance relativement positive.

Pour évaluer la force de la corrélation, nous effectuons une régression linéaire entre ces deux variables.  (si par commune, régression par pallier ??

```{r}
reg<-lm(donnees_agg$donnee_contexte_commune ~ donnees_agg$nb_repondants_commune)
plot(donnees_agg$nb_repondants_commune,donnees_agg$donnee_contexte_commune, main="Scatterplot du lien entre données de contexte et nombre de répondants", sub="Constatation de la corrélation",
  xlab="Nombre de répondants", ylab="Valeurs des données de contexte")
abline(reg, col = "red") 
summary(reg)
...
fonction_distance <- distance_Ward.D2_ponderee #distance_euclidienne_ponderee 
```
...
ggplot(df, aes(x = x_bin, y = y_bin, fill = as.factor(clusters))) +
  ...
  theme_void() 
  #+guides(fill = FALSE) + # désactiver la légende de remplissage
