...
Nous pouvons générer nos individus selon une loi uniforme dont les données sont gardées dans un tableau comme ceci: (la cellule suivante ne sera pas exécutée avec "eval = FALSE")
...
Pour générer aléatoirement des individus dans le plan avec une forte concentration dans certaines zones, nous définissons des zones métropolitaines. Par exemple, 4 sur tout le territoire.


Nous tout de même en sorte d'avoir au moins un habitant par commune. En 2020, on ne réfère environ que 6 communes sans population légale. Nous prenons donc une variance suffisamment large pour la distribution d'individus générés de sorte à recouvrir le territoire tout en imposant une certaine structure démographique

```{r,eval = FALSE }
# Diviser le plan en 4 régions rectangulaires pour placer nos grosses métropoles
x_mid <- x_max / 2
y_mid <- y_max / 2

# Placer un point au centre de chaque région rectangulaire
centers <- data.frame(
  x = c(x_mid - x_max / 4, x_mid + x_max / 4, x_mid - x_max / 4, x_mid + x_max / 4),
  y = c(y_mid - y_max / 4, y_mid + y_max / 4, y_mid + y_max / 4, y_mid - y_max / 4)
)

# Ajouter du bruit à ces 4 points pour obtenir une dispersion aléatoire des individus autour de ces centres
df <- data.frame(
  x = c(rnorm(n / 4, centers$x[1], x_max ), rnorm(n / 4, centers$x[2], x_max ),
        rnorm(n / 4, centers$x[3], x_max ), rnorm(n / 4, centers$x[4], x_max )),
  y = c(rnorm(n / 4, centers$y[1], y_max ), rnorm(n / 4, centers$y[2], y_max ),
        rnorm(n / 4, centers$y[3], y_max ), rnorm(n / 4, centers$y[4], y_max ))
)

# Si avoir 4 métropoles ne donnent pas assez de zones vides, nous pouvons ne garder que 2 métropoles en diagonale
...
Nous simulons le fait que des individus ont répondu à une enquête dans chaque carré les individus qui ont répondu à une enquête en effectuant un tirage de Bernoulli selon la variable correspondant aux données de contexte.

Nous créons donc une variable binaire "inclusion_enquete" qui renseigne si un individu est sélectionné dans l'enquête.

```{r}
# Effectuer le tirage de Bernoulli pour chaque carré
data$inclusion <- rbinom(nrow(data), 1, data$square_value)

# Afficher les carrés inclus dans l'échantillon
echantillon <- data[data$inclusion == 1, ]
```
...
On complète les communes sans répondants dans le tableau en précisant que nb_repondants_commune=0
...
Une critique possible est qu'une coloration de la grille n'est peut-être pas optimale ou suffisante en terme de visualisation. Il faudrait peut être définir des contours
