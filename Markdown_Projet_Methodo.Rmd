...
seuil_inertie <- 0.0000000000001 #0.000001
  ...
  if (groupes_voisins(groups,i,j)){ 
    ...
    if ( nb_repondant_agregat <= nb_repondants_max) { #contrainte de taille sur nombre de répondants
      ...
      dist_mat[i,i] <- dist_mat[i,i] <- Inf
        ...
        inertie = inertie_intra#inertie_intra/inertie_inter
...
"Nous récupérons l'indice correspondant au minimum du rapport d'inertie intra-inter classes". C'est ce que nous aurions fait avec une CAH classique. Seulement, le gain d'inertie intra est inévitable. Nous récupérons donc la dernière partition.
...
indice_dernier_groupes_retenu <- mength(liste_inertie)
```
