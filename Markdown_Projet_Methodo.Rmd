...
```{r,eval =FALSE}
# Nombre de carrés à tirer ou unités primaires, communes
n_squares_sample <- 10

# Tirage des carrés au hasard dans chaque commune
squares_sample <- sample(unique(donnees$ID), n_squares_sample)

# Sous-échantillonnage des individus pour chaque carré sélectionné
donnees_subsampled <- donnees %>%
  filter(carre_id %in% squares_sample) %>%
  group_by(carre_id) %>%
  sample_n(ceiling(0.5 * n()))

# Effectuer le tirage de Bernoulli selon les données de contexte pour chaque individu
donnees_subsampled$inclusion_enquete <- rbinom(nrow(donnees_subsampled), 1, donnees_subsampled$donnee_contexte_commune)
...
Nous définissons des zones très vides au niveau de l'enquête pour observer comment ces zones particulières seront agrégées. L'intérêt est d'observer s'il est possible d'obtenir un gros agrégat sans répondants. Que faire si cela arrive ?
...
Nous avons une structure des données de contexte qui affecte positivement avec le nombre de répondants
...
```{r}
distance_Ward.D2 <- function(mean1,mean2){
...
```{r}
distance_euclidienne_ponderee <- function(mean1_ponderee1, mean1_ponderee2) {
  sqrt(sum(weights * (mean1_ponderee - mean2_ponderee)^2))
}
```

Mais nous pouvons faire mieux puisque nous pouvons modifier la distance de Ward.D2 de sorte à ce qu'elle prenne en compte la pondération : 

```{r}
distance_Ward.D2_ponderee <- function(mean1,mean2){
  # Vecteurs des poids des 2 agrégats
  w1 <- data_group1$taille_commune
  w1 <- data_group1$taille_commune
  ...
  dist_between_groups <- ssd_all + w1*w2*(mean1-mean2)^2/(w1+w2)
  return(sqrt(dist_between_groups / ((length(data_group1)+length(data_group2)))))
...
Ainsi, nous pouvons normaliser par la taille des communes en agrégeant les individus par commune pour compter le nombre de points dans chaque carré. Il est intéressant de noter que *si l'information de la taille de la commune est indisponible, il est possible de l'estimer en sommant les poids de sondage de chaque enquêté par commune*. Sommer les poids de sondage dans chaque commune correspond aussi à une agrégation à l'échelle des communes.
...
```{r}
distance_between_groups <- function(groups,i,j,fonction_distance) {
  # Calcul de la distance entre deux groupes pour la méthode ward.d2
  ...
  mean1_ponderee <- sapply(data_group1, weighted.mean, w = data_group1$taille_commune)
  mean2_ponderee <- sapply(data_group2, weighted.mean, w = data_group2$taille_commune)
  mean_all_ponderee <- mean(c(mean1_ponderee , mean2_ponderee))
  ...
  mean1_ponderee_hat <- sapply(data_group1, weighted.mean, w = data_group1$somme_proba_inclusion_enquete)
  mean2_ponderee_hat <- sapply(data_group2, weighted.mean, w = data_group2$somme_proba_inclusion_enquete)
  mean_all_ponderee_hat <- mean(c(mean1_ponderee , mean2_ponderee))
  # Calcul de la distance entre chaque groupe selon la fonction de distance choisie
  distance <- fonction_distance(mean1_ponderee,mean2_ponderee)
...
fonction_distance <- distance_Ward.D2_ponderee
```
...
## Autre proposition de datavisualisation : distinction des gros et petits clusters
